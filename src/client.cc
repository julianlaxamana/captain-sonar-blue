...
int main() {
	...
	try {
		...
		cout << "Please enter IP address for the server (127.0.0.1 is normal): ";
		cin >> host;
		if (!cin) die();

		cout << "Please enter the port number to use (1300 is normal): ";
		cin >> port;
		if (!cin) die();

		tcp::iostream stream = client_start(host, port);
		if (!stream) die("Connection issue!\n");
		...
		int players = 2;
		stream >> players; 
		if (!stream || players < 2 || players > 6) die(); //FIXME: as far as I got
		...
		for (int i = 0; i < subs.size(); i++) { //Repeat the setup for each player
			...
			pause_screen();         //Wait for them to admire the screen before clearing it
			clear_screen();
			print_map();
